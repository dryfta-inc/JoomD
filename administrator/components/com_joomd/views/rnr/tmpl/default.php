...
<script type="text/javascript">
	...
	function get_pitems()
	{
		...
		$jd.ajax({
			  ...
			  type: "POST",
			  dataType:"json",
			  data: {'option':'com_joomd', 'view':'rnr', 'task':'get_pitems', 'filter_type':$jd('select[name="filter_type"]').val(), "<?php echo jutility::getToken(); ?>":1, 'abase':1},
			  success: function(res)	{
				
				if(res.result == 'success')	{
					$jd('select[name="filter_item"]').html(res.list);
					$jd('select[name="filter_item"]').multiselect("refresh");
				}
				else	{
...
<form action="index.php?option=com_joomd&view=rnr" method="post" name="adminlist">
...
<table width="100%">
	...
	<thead>
			...
			<th align="left" width="200">
				<?php echo JHTML::_('jdgrid.sort', JText::_('USER'), 'u.name', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
			</th>
            <th align="left" width="200">
            ...
            <th align="left" width="100">
				<?php echo JHTML::_('jdgrid.sort', JText::_('RATING'), 'i.rate', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
			</th>
	...
	<tfoot>
		<tr><td colspan="8"><div class="pagination" style="display:<?php echo $dis; ?>"><?php echo JHTML::_('jdgrid.loadmore', JText::_('LMORE'), $this->params->total, count($this->items) ); ?></div></td></tr>
	</tfoot>
