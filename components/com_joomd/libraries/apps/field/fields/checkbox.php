...
//Checkbox Field type since JoomD 2.3

class JoomdAppFieldCheckbox	{
	...
	function loadeditform($fieldid, $itemid, $params)
	{
		...
		foreach($options as $option)	{
			$option = trim($option);
			$html .= '<input type="checkbox" name="field_'.$field->id.'[]" id="field_'.$field->id.'" class="form_'.$field->cssclass.'" value="'.$option.'"';
	...
	function loadsearchform($fieldid, $itemid, $params)
	{
		...
		foreach($options as $option)	{
			$option = trim($option);
			$html .= '<input type="checkbox" name="field_'.$field->id.'[]" id="field_'.$field->id.'" class="search_'.$field->cssclass.'" value="'.$option.'" /> '.$option.'&nbsp;';
