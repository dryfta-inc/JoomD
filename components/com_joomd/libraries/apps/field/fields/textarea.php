...
//Textarea Field type since JoomD 2.3

class JoomdAppFieldTextarea	{
	...
	function loadeditform($fieldid, $itemid, $params)
	{
		...
		$rows = isset($custom->rows)?$custom->rows:8;
		$cols = isset($custom->cols)?$custom->cols:75;
	...
	function displayfieldvalue($itemid, $fieldid, $params)
	{
		...
		if($params['short'])
			return substr($value, 0, $params['char']).'...';
		else
			return $value;
