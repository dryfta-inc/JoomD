...
class modJoomd_itemsHelper
{
	...
	{	
		...
		$query = 'select * from #__joomd_'.$type->app.' as i join #__joomd_type'.$type->id.' as t on i.id=t.itemid';
		...
		if(count($cats))
			$where[] = ' id in (select itemid from #__joomd_'.$type->app.'_cat where catid in ('.implode(', ', $cats).'))';
